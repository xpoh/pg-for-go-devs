...
SELECT (random()*99+1)::int, (random()*99+1)::int, now(), md5(random()::text) from generate_series(1,1000);

INSERT INTO public.proc(id_test, id_param, value, raw_data, misc)
SELECT (random()*99+1)::int, (random()*99+1)::int, random()*2000-1000, null, md5(random()::text) from generate_series(1,1e6);